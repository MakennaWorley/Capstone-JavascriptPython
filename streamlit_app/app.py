...
from utils import SEED, display_categorical_pairs, display_eda, display_feature_vs_target, preprocess_data, train_and_evaluate_clf
...
st.caption('Example: http://localhost:8000/api/hello')
	...
	with data_tab3:
		...
		if processed_data is not None:
			...
			clf_models = [
				...
				(
					'Probabilistic Baseline (L2 Regularization)',
					LogisticRegression,
					X_train,
					y_clf_train,
					X_test,
			...
			for name, model_class, X_tr, y_tr, X_te, y_te, params in clf_models:
				...
				results = train_and_evaluate_clf(model_class, X_tr, y_tr, X_te, y_te, name, **params)
				...
				clf_results_display.append(
					{'Model': results['model'], 'Test Accuracy': f'{results["accuracy"]:.4f}', 'Test AUC': f'{results["auc"]:.4f}'}
				)
				st.markdown('---')
