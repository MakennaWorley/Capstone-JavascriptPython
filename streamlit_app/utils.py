...
def get_vars(df, exclude_cols=None):
	...
	# Return everything
	# --------------------
	return {'true_numerical_vars': true_numerical_vars, 'true_categorical_vars': true_categorical_vars, 'all_vars': all_vars}
...
def display_categorical_pairs(data):
	...
	st.text(
		"For right now, I am only graphing the first 3 columns since this is a time-consuming process and I haven't fully"
	...
	fig, axes = plt.subplots(n_categorical_vars, n_categorical_vars, figsize=(n_categorical_vars * plot_size, n_categorical_vars * plot_size))
	...
	for i in range(n_categorical_vars):
		for j in range(n_categorical_vars):
			...
			# --- Off-Diagonal (i != j): Pairwise Relationship (Heatmap) ---
				...
				sns.heatmap(contingency_table, annot=True, fmt='.2f', cmap='YlGnBu', cbar=False, ax=axes[i, j], linewidths=0.5, linecolor='gray')
...
def display_feature_vs_target(data, target_name='y'):
	...
	for i, feature_name in enumerate(dosage_cols):
		...
		sns.countplot(x=data[feature_name], hue=data[target_name], data=data, palette='Pastel2', order=data[feature_name].value_counts().index, ax=ax)
...
def preprocess_data(data, target_col='y'):
	...
	X_train_scaled_df = pd.DataFrame(X_train_scaled_num, columns=numerical_cols, index=X_train_resampled.index)

	# Transform test numerical features
	X_test_scaled_num = scaler.transform(X_test[numerical_cols])
	X_test_scaled_df = pd.DataFrame(X_test_scaled_num, columns=numerical_cols, index=X_test.index)
