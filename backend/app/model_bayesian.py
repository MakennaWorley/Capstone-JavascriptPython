...
def _run_fold_parallel(args):
	...
	fold_idx, train_idx, val_idx, X_all, y_all, groups_all = args
...
# -----------------------------
# Bayesian Categorical Dosage (Softmax)
# -----------------------------
...
def train_with_cross_val(base_name, prep_cfg, n_splits=5, models_dir='models'):
	"""
	...
	for i, (t_idx, v_idx) in enumerate(kf.split(X_all)):
		fold_args.append((i, t_idx, v_idx, X_all, y_all, groups_all))
	...
	final_model = BayesianCategoricalDosageClassifier(cores=1)
...
def train_eval_one(
	...
	*,
	...
	model = train_with_cross_val(val_base, prep_cfg)
